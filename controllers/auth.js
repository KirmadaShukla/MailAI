...
const authGmail=async (req,res)=>{
 const userIdAsEmail = req.query.userId; // This is the email for login_hint
  if (!userIdAsEmail) {
    return res.status(400).json({ success: false, error: 'User ID (email) required in query' });
  }
...
const authCallback=async(req,res)=>{
      const { code, state } = req.body; // Renamed userId from body to state for clarity, as it is the state param
  ...
  if (!userIdFromState) {
    ...
    return res.status(400).json({
      ...
      details: 'The OAuth callback did not include the required state parameter (userId/email)'
    });
  }
  ...
  try {
    ...
    if (!user) {
      ...
      console.warn(`User ${userIdFromState} found, but email field was incorrect: ${user.email}. Correcting to ${userIdFromState}.`);
    ...
    const verificationClient = new google.auth.OAuth2(
      config.gmail.clientId,
      config.gmail.clientSecret,
      config.gmail.redirectUri
    );
    verificationClient.setCredentials(tokens);

    const gmail = google.gmail({ version: 'v1', auth: verificationClient });
    ...
    if (authenticatedEmail.toLowerCase() !== userIdFromState.toLowerCase()) {
      console.warn(`Email mismatch for user ${userIdFromState}: Expected ${userIdFromState}, but authenticated with ${authenticatedEmail}`);
      try {
        if (tokens.access_token) { // Only try to revoke if there's an access token
          await verificationClient.revokeToken(tokens.access_token);
          console.log(`Token revoked for ${authenticatedEmail} due to email mismatch.`);
        }
      } catch (revokeError) {
        console.error(`Failed to revoke token for ${authenticatedEmail}:`, revokeError.message);
      }
    ...
    await user.save();
    console.log(`Tokens saved for user ${userIdFromState} (email: ${user.email})`);

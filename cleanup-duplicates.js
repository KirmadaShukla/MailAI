...
const { cleanupDuplicateLabels } = require('./services/parallelProcessingService');
const config = require('./config/config');

async function runCleanup() {
  try {
    console.log('🧹 Starting cleanup of duplicate Maily/ labels...');
    console.log('📧 This will remove extra Maily/ labels from emails that have multiple ones');
    console.log('✅ Each email will keep only ONE Maily/ label (the first one found)');
    console.log('');

    // Use global refresh token for cleanup (you can modify this for specific user)
    const userAuth = {
    ...
    console.log('==================');
    console.log(`🔍 Emails with duplicate labels found: ${result.duplicatesFound}`);
    ...
    if (result.cleaned > 0) {
      ...
      console.log('');
      console.log('🔄 You can now run processing again and it should work correctly!');
